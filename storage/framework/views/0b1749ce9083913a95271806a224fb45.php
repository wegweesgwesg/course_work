...
<?php $__env->startSection('title', e($build->name) . ' — Каталог сборок — bit.sp'); ?>
...
<main class="build-detail-page">
    <a href="<?php echo e(route('builds.index')); ?>" class="back-link">&larr; Назад к каталогу</a>

    <div class="bd-header">
        <div class="bd-title"><?php echo e($build->name); ?></div>
    </div>
    <div class="bd-author">
        Автор: <?php echo e($build->user->first_name ?? 'Неизвестно'); ?>

        <?php if(in_array($build->user->role ?? '', ['admin','content_manager','warehouse_manager'])): ?>
            <span class="role-badge">Администрация</span>
        <?php endif; ?>
        &middot; <?php echo e($build->created_at->format('d.m.Y H:i')); ?>

    </div>

    <?php if($build->description): ?>
        <div class="bd-desc"><?php echo e($build->description); ?></div>
    <?php endif; ?>

    <?php if(auth()->guard()->check()): ?>
        <?php if(Auth::user()->isPrivileged()): ?>
            <div class="bd-admin-actions">
                <button id="admin-delete-build" data-id="<?php echo e($build->id); ?>">Удалить сборку</button>
            </div>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <div class="bd-components">
        ...
        ?>
        <?php if(is_array($buildData)): ?>
            <?php $__currentLoopData = $buildData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slot => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $price = $item['price'] ?? 0;
                    $total += $price;
                ?>
                <div class="bd-comp-item">
                    <div class="bd-comp-img">
                        <?php if(!empty($item['main_image_path'])): ?>
                            <img src="/<?php echo e(e($item['main_image_path'])); ?>" alt="">
                        <?php else: ?>
                            📦
                        <?php endif; ?>
                    </div>
                    <div class="bd-comp-info">
                        <div class="bd-comp-slot"><?php echo e(resolveSlotLabel($slot, $slotLabels)); ?></div>
                        <div class="bd-comp-name"><?php echo e($item['name'] ?? $item['product_id'] ?? '—'); ?></div>
                        <div class="bd-comp-price"><?php echo e(number_format($price, 0, '.', ' ')); ?> ₽</div>
                    </div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        <?php endif; ?>
        <div class="bd-total">Итого: <?php echo e(number_format($total, 0, '.', ' ')); ?> ₽</div>
    </div>
    ...
    <div class="bd-voting">
        <button class="bd-vote-btn<?php echo e($userVote === 1 ? ' active-up' : ''); ?>" id="vote-up" <?php echo e(Auth::guest() ? 'disabled' : ''); ?>>👍</button>
        <div class="bd-score <?php echo e($build->score > 0 ? 'positive' : ($build->score < 0 ? 'negative' : 'neutral')); ?>" id="bd-score">
            <?php echo e($build->score > 0 ? '+' : ''); ?><?php echo e($build->score); ?>

        </div>
        <button class="bd-vote-btn<?php echo e($userVote === -1 ? ' active-down' : ''); ?>" id="vote-down" <?php echo e(Auth::guest() ? 'disabled' : ''); ?>>👎</button>
    ...
    <div class="bd-comments">
        <h3>Комментарии (<?php echo e($build->comments->count()); ?>)</h3>

        <div id="comments-list">
            <?php $__currentLoopData = $build->comments->sortByDesc('created_at'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="bd-comment" id="comment-<?php echo e($c->id); ?>">
                    <div class="bd-comment-header">
                        <div class="bd-comment-author"><?php echo e($c->user->first_name ?? 'Аноним'); ?></div>
                        <div style="display:flex;align-items:center;gap:8px;">
                            <div class="bd-comment-date"><?php echo e($c->created_at->format('d.m.Y H:i')); ?></div>
                            <?php if(auth()->guard()->check()): ?>
                                <?php if(Auth::user()->isPrivileged()): ?>
                                    <button class="bd-comment-del" data-comment-id="<?php echo e($c->id); ?>">Удалить</button>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="bd-comment-text"><?php echo e($c->text); ?></div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
(function() {
    ...
    var BUILD_ID = <?php echo e($build->id); ?>;
    ...
    if (applyBtn) {
        applyBtn.addEventListener('click', function(e) {
            e.preventDefault();
            var buildData = <?php echo json_encode($build->build_data, 15, 512) ?>;
            localStorage.setItem('apply_build_data', JSON.stringify(buildData));
            window.location.href = '<?php echo e(route("configurator")); ?>';
        });
    }
})();
</script>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Files\Study\2026\kursach_2\pc-configurator\resources\views/builds/show.blade.php ENDPATH**/ ?>